...
public:
  ...
  /**
   * @brief Set the number of RTS retransmissions
   *
   * @param rtsretries The number of RTS retransmissons
   */
  void SetRtsRetries (uint8_t retries);

  /**
   * @brief Get the number of RTS retransmissions
   *
   * @returns The number of RTS retransmissons
   */
  uint8_t GetRtsRetries (void) const;

  /**
   * @brief Set the number of data retransmissions
   *
   * @param dataretries The number of data retransmissons
   */
  void SetDataRetries (uint8_t retries);

  /**
   * @brief Get the number of data retransmissions
   *
   * @returns The number of RTS retransmissons
   */
  uint8_t GetDataRetries (void) const;
  ...
  /**
   * @brief checks if the RTS retries param is set
   *
   * @returns true if the RTS retries is set
   */
  bool IsRtsRetriesSet (void);

  /**
   * @brief checks if the DATA retries param is set
   *
   * @returns true if the Data retries is set
   */
  bool IsDataRetriesSet (void);

  /**
   * @brief checks if the TX power param is set
   *
   * @returns true if the TX power is set
   */
  bool IsTxPowerSet (void);
...
private:
  enum {
    ...
    RADIOTAP_TX_FLAGS          = 0x00008000,
    RADIOTAP_RTS_RETRIES       = 0x00010000,
    RADIOTAP_DATA_RETRIES      = 0x00020000,

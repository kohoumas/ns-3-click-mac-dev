...
RadiotapHeader::Serialize (Buffer::Iterator start) const
{
  ...
  // TX power level
  //
  if (m_present & RADIOTAP_DBM_TX_POWER) // bit 8
    {
  ...
  // RX Flags
  //
  if (m_present & RADIOTAP_RX_FLAGS) // bit 9
    {
      start.WriteU8 (m_rxFlags);
    }

  //
  // TX Flags
  //
  if (m_present & RADIOTAP_TX_FLAGS) // bit 10
    {
      start.WriteU8 (m_txFlags);
    }

  //
  // Number of RTS retransmissions
  //
  if (m_present & RADIOTAP_RTS_RETRIES) // bit 11
    {
      start.WriteU8 (m_rtsRetries);
    }

  //
  // Number of data retransmissions
  //
  if (m_present & RADIOTAP_DATA_RETRIES) // bit 12
    {
      start.WriteU8 (m_dataRetries);
    }
}
...
RadiotapHeader::Print (std::ostream &os) const
{
  ...
  os << " tsft=" << m_tsft
     ...
     << " txPower=" << (uint16_t) m_txPower
     << " rts retries=" << (int16_t) m_rtsRetries
     << " data retries=" << (int16_t) m_dataRetries;
}
...
void
RadiotapHeader::SetRtsRetries (uint8_t retries)
{
  NS_LOG_FUNCTION (this << retries);
  m_rtsRetries = retries;
}

uint8_t
RadiotapHeader::GetRtsRetries (void) const
{
  NS_LOG_FUNCTION (this);
  return m_rtsRetries;
}
...
void
RadiotapHeader::SetDataRetries (uint8_t retries)
{
  NS_LOG_FUNCTION (this << retries);
...
uint8_t
RadiotapHeader::GetDataRetries (void) const
{
  NS_LOG_FUNCTION (this);
...
void
RadiotapHeader::SetTxPower (uint8_t txPower)
{
...
uint8_t
RadiotapHeader::GetTxPower (void) const
{
  NS_LOG_FUNCTION (this);
  return m_txPower;
}
...
bool
RadiotapHeader::IsRtsRetriesSet (void)
{
  return (m_present & RADIOTAP_RTS_RETRIES);
}

bool
RadiotapHeader::IsDataRetriesSet (void)
{
  return (m_present & RADIOTAP_DATA_RETRIES);
}

bool
RadiotapHeader::IsTxPowerSet (void)
{
  return (m_present & RADIOTAP_DBM_TX_POWER);
}

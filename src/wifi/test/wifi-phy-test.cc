...
private:
  void Send (void);
  void Receive (Ptr<Packet> p, double snr, WifiMode mode, enum WifiPreamble preamble, RadiotapHeader radiotaphdr);
...
PsrExperiment::Send (void)
{
  ...
  m_tx->SendPacket (p, mode, WIFI_PREAMBLE_SHORT, m_input.txPowerDbm);
}

void
PsrExperiment::Receive (Ptr<Packet> p, double snr, WifiMode mode, enum WifiPreamble preamble, RadiotapHeader radiotaphdr)
{
  m_output.received++;
}
...
private:
  ...
  void Receive (Ptr<Packet> p, double snr, WifiMode mode, WifiPreamble preamble, RadiotapHeader radiotaphdr);
...
CollisionExperiment::SendA (void) const
{
  ...
  m_txA->SendPacket (p, WifiMode (m_input.txModeA),
                     WIFI_PREAMBLE_SHORT, m_input.txPowerDbmA);
}
...
CollisionExperiment::SendB (void) const
{
  ...
  m_txB->SendPacket (p, WifiMode (m_input.txModeB),
                     WIFI_PREAMBLE_SHORT, m_input.txPowerDbmB);
}

void
CollisionExperiment::Receive (Ptr<Packet> p, double snr, WifiMode mode, WifiPreamble preamble, RadiotapHeader radiotaphdr)
{
  FlowIdTag tag;

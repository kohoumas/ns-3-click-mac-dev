...
WifiNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  NS_LOG_DEBUG (this << packet << " Flag: " << (bool)m_monitor);
...
WifiNetDevice::ForwardUp (Ptr<Packet> packet, Mac48Address from, Mac48Address to)
{
  ...
  if (!m_monitor)
    ...
    {
      // create packet copy to strip Radiotap and WifiMac headers before LLC
      Ptr<Packet> pCopy = packet->Copy (); 
      RadiotapHeader rtHeader;
      WifiMacHeader wmHeader;
      pCopy->RemoveHeader (rtHeader);
      pCopy->RemoveHeader (wmHeader);
...
{
  m_monitor = true; // Must be set only if WifiNetDevice has a MonitorWifiMac underneath
}

bool
WifiNetDevice::IsMonitorMode (void) const
{
  return m_monitor;
}

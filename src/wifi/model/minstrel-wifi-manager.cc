                                    ...
                                    uint32_t size)
{
  ...
  UpdateStats (station);
  return WifiTxVector (GetSupported (station, station->m_txrate), GetDefaultTxPowerLevel (), st->m_slrc);
}
...
MinstrelWifiManager::DoGetRtsTxVector (WifiRemoteStation *st)
{
  ...
  return WifiTxVector (GetSupported (station, 0), GetDefaultTxPowerLevel (), st->m_ssrc);
}

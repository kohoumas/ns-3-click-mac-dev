...
rxPacket:
  WifiMacTrailer fcs;
  //packet->RemoveTrailer (fcs); // kohoumas : Click later will remove the FCS trailer, so ns-3 should not remove it now.
...
MacLow::SendCtsAfterRts (Mac48Address source, Time duration, WifiMode rtsTxMode, double rtsSnr)
{
  ...
  cts.SetNoRetry ();
  cts.SetAddr1 (source);
  duration -= GetCtsDuration (source, ctsTxVector.mode);
  duration -= GetSifs ();

...
Ipv4L3ClickProtocol::AddInterface (Ptr<NetDevice> device)
{
  ...
  Ptr<Node> node = GetObject<Node> ();
  node->RegisterProtocolHandler (MakeCallback (&Ipv4L3ClickProtocol::Receive, this),
                                 0, device); //Ipv4L3ClickProtocol::PROT_NUMBER, device); // kohoumas : accept any protocol number (not only 0x0800) like 0x0941, 0x0943, etc.
  node->RegisterProtocolHandler (MakeCallback (&Ipv4L3ClickProtocol::Receive, this),
...
Ipv4L3ClickProtocol::SendDown (Ptr<Packet> p, int ifid)
{
  ...
  if (DynamicCast<WifiNetDevice> (netdev) &&
      DynamicCast<WifiNetDevice> (netdev)->IsMonitorMode ())
    {
      netdev->Send (p, Mac48Address (), (uint8_t)0);
    }
  else
    {
                               ...
                               const Address &to, NetDevice::PacketType packetType)
{
  ...
  if (!(DynamicCast<WifiNetDevice> (device) &&
        DynamicCast<WifiNetDevice> (device)->IsMonitorMode ()))
    {
